...
void check_mouse(XEvent *e)
{
	...
	float bullet_pos = 20.0f;
	float bullet_vel = 6.0f;
	float mid = 100;
	float add_speed = 15.0f;
	float angle = .05f;
		...
		if (e->xbutton.button==1) {
			...
			if (ts > 0.1) {
				...
				if (g.nbullets < MAX_BULLETS) {
					...
					Flt rad = ((g.ship.angle+right_angle) / circle) * PI * 2.0;
					//convert angle to a vector
					Flt xdir = cos(rad);
					Flt ydir = sin(rad);
					b->pos[0] += xdir*bullet_pos;
					b->pos[1] += ydir*bullet_pos;
					b->vel[0] += xdir*bullet_vel + rnd()*0.1;
					b->vel[1] += ydir*bullet_vel + rnd()*0.1;
					b->color[0] = 1.0f;
					b->color[1] = 1.0f;
					b->color[2] = 1.0f;
					++g.nbullets;
	...
	if (e->type == MotionNotify) {
		if (savex != e->xbutton.x || savey != e->xbutton.y) {
			...
			if (xdiff > 0) {
				...
				g.ship.angle += angle * (float)xdiff;
				if (g.ship.angle >= circle)
					g.ship.angle -= circle;
			}
			else if (xdiff < 0) {
				g.ship.angle += angle * (float)xdiff;
			...
			x11.set_mouse_position(mid, mid);
			savex = savey = mid;
		}
	}
}
...
void buildAsteroidFragment(Asteroid *ta, Asteroid *a)
{
	...
	ta->color[0] = (float)rand()/(float)RAND_MAX;
	ta->color[1] = (float)rand()/(float)RAND_MAX;
	ta->color[2] = (float)rand()/(float)RAND_MAX;
